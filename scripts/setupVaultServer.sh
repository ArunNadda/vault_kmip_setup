...
license_path = "/vagrant/license.hcl"
storage "raft" {
  path    = "/vault/transit/"
...
echo "KMIP_CONNECTION_DEBUGGING=1" >> /var/transit/vtoken.tok
echo "KMIP_REQUEST_DEBUGGING=1" >> /var/transit/vtoken.tok
echo "KMIP_RESPONSE_DEBUGGING=1" >> /var/transit/vtoken.tok
echo "KMIP_DECODING_DEBUGGING=1">> /var/transit/vtoken.tok
echo "KMIP_INDEX_DEBUGGING=1" >> /var/transit/vtoken.tok
...
then
  ...
  # set kmip only on vaults0
  /vagrant/scripts/configure_kmip.sh
